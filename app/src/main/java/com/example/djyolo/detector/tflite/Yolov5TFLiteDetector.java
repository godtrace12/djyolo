...
public class Yolov5TFLiteDetector implements ITFliteDetector {

    private final Size INPNUT_SIZE = new Size(320, 320);
    private int[] OUTPUT_SIZE = new int[]{1, 6300, 85}; //10,  yolo：85  手势：15
    ...
    private final String MODEL_YOLOV5S = "yolov5s-fp16-320.tflite"; //coco128自训练后导出的模型
    private final String MODEL_YOLOV5N =  "yolov5s-fp16-320.tflite";
    ...
    public void initialModel(Context activity) {
        ...
        try {
            ...
            Log.d("tflite", "模型输出结构: "+Arrays.toString(outputShape));
...
//        Log.d("dj", "detect: nmsRecSize="+nmsRecognitions.size()+" filterRecSize="+nmsFilterBoxDuplicationRecognitions.size());
        // 更新label信息
//        Log.d("dj", "detect: "+Arrays.toString(associatedAxisLabels.toArray()));
        for(Recognition recognition : nmsFilterBoxDuplicationRecognitions){
            int labelId = recognition.getLabelId();
            if(recognition != null){
                Log.d("dj", "detect: labelId="+labelId);
            }else{
                Log.d("dj", "detect: recognition is null");
            }
            String labelName = associatedAxisLabels.get(labelId);
            Log.d("dj", "detect: curLabelName="+labelName+" curLabelId="+labelId);
            recognition.setLabelName(labelName);
        }
        Log.d("dj", "detect: result size="+nmsFilterBoxDuplicationRecognitions.size());
