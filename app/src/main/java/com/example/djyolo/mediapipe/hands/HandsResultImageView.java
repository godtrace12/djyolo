...
public class HandsResultImageView extends AppCompatImageView {
  ...
  public void setHandsResult(HandsResult result) {
    ...
    for (int i = 0; i < numHands; ++i) {
      LandmarkProto.NormalizedLandmarkList markList = result.multiHandLandmarks().get(i);
      isVictory = HandGestureDetector.isVictoryGesture(markList);
      long timeBefore = System.currentTimeMillis();
      HaGRIDGestureClassifier.GestureResult hagridResult = HaGRIDGestureClassifier.recognizeGesture(markList,null);
  ...
  private void drawLandmarksOnCanvas(
      ...
      int width,
      int height,
      boolean isVictory,HaGRIDGestureClassifier.GestureResult gesRecognizeResult) {
    // Draw connections.
    ...
    String recResult = gesRecognizeResult.gesture.name()+"-"+gesRecognizeResult.confidence;
//    if(isVictory){
      Paint textPaint = new Paint();
      textPaint.setColor(LEFT_HAND_LANDMARK_COLOR);
      textPaint.setTextSize(40f);
      textPaint.setTextAlign(Paint.Align.LEFT);
      canvas.drawText(recResult,100,100,textPaint);
      Log.e(TAG, "drawLandmarksOnCanvas: is ok");
//    }
